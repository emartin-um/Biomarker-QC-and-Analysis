...
meta_orig_path  <- file.path(DATASETS_DIR, "U19_Alamar_metadata_2026Apr27.csv")
meta_new_path   <- file.path(DATASETS_DIR, "OtherProjects_metadata_03052026.xlsx")
combined_path   <- file.path(DATASETS_DIR, "U19_OtherProjects_metadata_combined_n2850_2026Apr.csv")
...
meta_orig <- read_csv(meta_orig_path, show_col_types = FALSE) |>
  mutate(metadata_source = "U19_2026Apr27")

meta_new <- read_xlsx(meta_new_path) |>
  mutate(
    # Force Group to character — read_xlsx reads an all-NA column as logical,
    # which causes type conflicts when bind_rows() joins with U19's character Group.
    Group = as.character(Group),
    # Trim whitespace from ancestry columns — Excel cells can carry trailing
    # spaces on multi-word values (e.g. "BL WH ", "WH MU ") that break rule
    # matching in the pipeline.
    across(c(Race, Ethnicity), str_trim),
    metadata_source = "OtherProjects_03052026"
  )
...
#
# fs::file_copy(combined_path, "input_files/U19_OtherProjects_metadata_combined_n2850_2026XXX.csv")
