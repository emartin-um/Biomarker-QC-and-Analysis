...
6.  **Covariate Groupings** *(optional: `include_derived_covariates`, default TRUE)* — explore distributions (6a), define configs (6b), preview decisions (6c), apply + validate + filter (6d); produces CDX_collapsed, Ancestry, APOE4_carrier, APOE4_count; rows excluded by default when either covariate is NA (`filter_na_covars`)
...
if (INCLUDE_DERIVED_COVARS) {
  ...
  # APOE4 carrier variables (simple computation — not config-based)
  filtered_combined <- filtered_combined %>%
    mutate(
      APOE4_carrier = grepl("4", APOE.geno),
      APOE4_count   = case_when(
        grepl("4.*4", APOE.geno) ~ 2L,
        grepl("4",    APOE.geno) ~ 1L,
        !is.na(APOE.geno)        ~ 0L,
        TRUE                     ~ NA_integer_
      )
    )
  ...
  write_csv(filtered_combined, file.path(OUTPUT_DIR, "filtered", "filtered_combined_post_QC.csv"))
  cat("\n`filtered_combined_post_QC.csv` updated with CDX_collapsed, Ancestry, APOE4_carrier, APOE4_count.\n\n")
