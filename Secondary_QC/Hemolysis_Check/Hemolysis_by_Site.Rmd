...
# Updated 2026-05-05 for v4 data — added metatada_source, newsamp, CDX_collapsed,
# Ancestry, APOE_WGS_norm, APOE_WGS_changed, APOE.geno_final, APOE4_carrier,
# APOE4_count, APOE_WGS, APOE4 (NULISA), E4_dose, European_anc_pct.
# Updated 2026-06-21 — fixed "metatada_source" typo (real col is "metadata_source")
# and added ResultQC, ID2, is_repeat_subject, is_most_recent (non-numeric QC/ID cols
# that otherwise fall into biomarker_cols and crash lm()).
# Updated 2026-06-23 (50-plate combined) — added gp_APOE, n_visits, subject_visit_n,
# AlamarWarning, PrimaryQC: NUMERIC QC/ID cols not caught by the non-numeric guard, which
# otherwise leak into biomarker_cols (134 vs the true 131) and contaminate the Site-effect FDR.
metadata_cols <- c("metadata_source", "ResultQC", "ID2",
                   "is_repeat_subject", "is_most_recent",
                   "RUN", "SAMPLE_ALIQUOT", "SAMPLE", "Record_ID",
                   "newsample", "newsamp",
                   "Group", "Ethnicity", "Race", "sex", "age_at_subject",
                   "CDX", "CDX_collapsed", "Case_Control", "AOO", "Years_Onset",
                   "APOE.geno", "APOE.geno_final", "APOE_WGS", "APOE_WGS_norm",
                   "APOE_WGS_changed", "APOE4_count", "APOE4_carrier", "E4_dose",
                   "gp_APOE", "n_visits", "subject_visit_n", "AlamarWarning", "PrimaryQC",
                   "Ancestry", "European_anc_pct",
...
# Guard: biomarkers must be numeric. Drops any stray non-numeric column that isn't
# in metadata_cols (e.g. character QC/ID flags) — modelling such a column as an lm()
# response throws "NA/NaN/Inf in 'y'". Robust to future schema additions.
non_numeric_dropped <- biomarker_cols[!vapply(df[biomarker_cols], is.numeric, logical(1))]
biomarker_cols <- biomarker_cols[vapply(df[biomarker_cols], is.numeric, logical(1))]
if (length(non_numeric_dropped))
  cat("Dropped non-numeric (non-biomarker) columns:", paste(non_numeric_dropped, collapse = ", "), "\n")
...
if (file.exists(rep_file)) {
  ...
  if (length(hemo_in_reps) >= 3) {
    ...
    if ("HBA1" %in% names(df_hemo)) {
      ...
      # Screen ALL biomarkers against HBA1. The main data uses underscore names (A_38, pTau_217)
      # while the replicate file uses Greek-beta/hyphen names (Aβ38, pTau-217); a plain intersect()
      # silently dropped the amyloid/pTau markers (the most important to screen). Normalize names so
      # Tech_CV is matched from reps when available, but test every biomarker that has HBA1 data.
      .norm <- function(x) toupper(gsub("[^A-Za-z0-9]", "", gsub("β", "", x)))
      rep_map <- setNames(names(df_reps), .norm(names(df_reps)))   # normalized name -> rep colname
      biomarkers_to_test <- intersect(biomarker_cols, names(df_hemo))

      # Calculate correlation with HBA1 for each biomarker
      hba1_cors <- map_df(biomarkers_to_test, function(biom) {
        test_data <- df_hemo %>%
        ...
        if (nrow(test_data) >= 50) {
          ...
          # Also get tech CV if available (match rep column by normalized name)
          rep_col  <- rep_map[[.norm(biom)]]
          rep_vals <- if (!is.null(rep_col)) df_reps[[rep_col]] else NULL
